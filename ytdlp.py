...
class YtDlpManager:
    ...
    def install_update(self):
        log = []
        try:
            # yt-dlp - use longer timeout for slow connections
            try:
                startupinfo = None
                if os.name == 'nt':
                    startupinfo = subprocess.STARTUPINFO()
                    startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                res = subprocess.run(['pip', 'install', 'yt-dlp', '--upgrade'], capture_output=True, text=True, encoding='utf-8', errors='ignore', startupinfo=startupinfo, timeout=120)
                if res.returncode == 0:
                    # Extract version from output if possible
                    log.append("✅ yt-dlp updated successfully")
                else:
                    log.append(f"❌ yt-dlp failed: {res.stderr.strip()[:200] if res.stderr else 'Unknown error'}")
            except subprocess.TimeoutExpired:
                log.append("❌ yt-dlp update timed out (try running: pip install yt-dlp --upgrade manually)")
            except Exception as e:
                log.append(f"❌ yt-dlp failed: {str(e)}")
    ...
    def fetch_info(self, url):
        try:
            ...
            # Use sys.executable to ensure we use the pip-installed version with JS runtime support
            cmd = [sys.executable, '-m', 'yt_dlp', '--dump-json', '--no-warnings', '--no-playlist', '--ignore-errors', url]
    ...
    def download(self, url, path, opts, progress_callback):
        self.cancel_flag = False
        # Use sys.executable to ensure we use the pip-installed version with JS runtime support
        cmd = [sys.executable, '-m', 'yt_dlp', '-o', os.path.join(path, '%(title)s.%(ext)s'), '--newline', '--no-playlist', url]
        ...
        if d_type == "audio":
            ...
            if d_type == "video":
                # Video only - simplified with fallback
                if h:
                    f_str = f'bestvideo[height<={h}]/bestvideo/best'
                else:
                    f_str = 'bestvideo/best'
                cmd.extend(['-f', f_str, '--merge-output-format', fmt])
            else:
                # Both video + audio - prefer m4a audio for MP4 compatibility (avoid opus)
                if h:
                    f_str = f'bestvideo[height<={h}]+bestaudio[ext=m4a]/bestvideo[height<={h}]+bestaudio/bestvideo+bestaudio/best'
                else:
                    f_str = 'bestvideo+bestaudio[ext=m4a]/bestvideo+bestaudio/best'
                cmd.extend(['-f', f_str, '--merge-output-format', fmt])
        ...
        try:
            ...
            if "format is not available" in full_err:
                ...
                simple_cmd = [sys.executable, '-m', 'yt_dlp', '-o', os.path.join(path, '%(title)s.%(ext)s'), '--newline', '--no-playlist', '-f', 'best', url]
